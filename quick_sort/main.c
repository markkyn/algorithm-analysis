...
int32_t hoare_rand(int32_t *vector, int32_t i, int32_t j)
{
    ...
    return hoare_partition(vector, i, j);
}
...
int32_t lomuto_median(int32_t *vector_ptr, int32_t i, int32_t j)
{
    // aka: LM(x)
    call_count++;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < vectors_count; i++)
    {
        ...
        call_count = 0;
        sort[HA].type = "HA";
        recursive_quicksort(ha_ptr, 0, vector_len - 1, LP);
        sort[HA].call_count = call_count;

        // TODO: SORT analysis call_count

        fprintf(output_fp, "%d: N(%d)", i, vector_len);

        for (int j = 0; j < 6; j++) // TODO: enum to str
