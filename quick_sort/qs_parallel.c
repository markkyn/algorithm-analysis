...
/* ------ QuickSort ------ */
/* ----------------------- */
void *recursive_quicksort(void *arg)
{
    SortParam_t *args = (SortParam_t *)arg;

    int32_t *vector_ptr, low, high;
    Type type;

    vector_ptr = args->pointer;
    printf("\n AQUIII \n\n");
    low = args->low;
    high = args->high;
    //call_count++;
...
int main(int argc, char *argv[])
{
    ...
    threads = (pthread_t *)malloc(6 * sizeof(pthread_t));
    ...
    for (int i = 0; i < vectors_count; i++)
    {
        ...
        SortParam_t *arg = malloc(sizeof(SortParam_t));
        arg->pointer = lm_ptr;
        arg->type = LM;
        arg->low = 0;
        arg->high = (vector_len - 1);

        pthread_create(&sort[LM].thread, NULL, recursive_quicksort, (void *)arg);
        ...
        pthread_join(sort[LM].thread, NULL);
